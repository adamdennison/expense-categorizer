...
const ExpenseCategorizer = () => {
  ...
  const saveData = async (txns, cats) => {
    try {
      await Promise.all([
        window.storage.set('transactions', JSON.stringify(txns)),
        window.storage.set('categories', JSON.stringify(cats))
      ]);
    } catch (error) {
  ...
  const confirmMapping = () => {
    ...
    Papa.parse(file, {
      ...
      dynamicTyping: true,
      complete: (results) => {
        const newTransactions = results.data.map((row, idx) => ({
          id: Date.now() + idx,
